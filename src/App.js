...
function App() {
  ...
  const status = ['In progress', 'Todo', 'Backlog', 'Done', 'Cancelled'];
    ...
    switch (groupBy) {
      case 'priority': {
        setGroups(priority.map((pr, i) => ({
          title: pr,
          group: data.filter(d => d.priority === i).sort((a, b) => (a[orderBy] < b[orderBy]) * -1
          )
        })).reverse());
        break;
      }
      case 'status': {
        setGroups(status.map(st => ({
          title: st,
          group: data.filter(d => d.status === st).sort((a, b) => (a[orderBy] < b[orderBy]) * -1
          )
        })));
        break;
      }
      case 'user': {
        setGroups(user.map(u => ({
          title: u.name,
          group: data.filter(d => d.userId === u.id).sort((a, b) => (a[orderBy] < b[orderBy]) * -1
          )
        })));
        break;
      }
  ...
  return (
    ...
    <div>
      <button id='filter' onClick={() => setVisible(visible ? false : true)}> Display </button><br/>
      {visible && <div id='filter-box'>
        <b>Group By:</b> {['status', 'priority', 'user'].map(g => <>
        <input name={'group-by'} value={g} type='radio' checked={g === groupBy} onClick={() => {setGroupBy(g);
          localStorage.setItem('group-by',g)}
        }/> <label for={g}>{g[0].toUpperCase() + g.slice(1)}</label>
        &emsp;</>)}<br />
        <b>Order By:</b> {['title', 'priority'].map(o => <>
        <input name={'order-by'} value={o} type='radio' checked={o === orderBy} onClick={() => {setOrderBy(o);
          localStorage.setItem('order-by',o)}
        }/> <label for={o}>{o[0].toUpperCase() + o.slice(1)}</label>
        &emsp;</>)}
      </div>}
